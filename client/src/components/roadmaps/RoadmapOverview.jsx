import { Briefcase, CheckCircle2, CircleDashed, Circle } from "lucide-react";

export default function RoadmapOverview({ roadmap }) {
  ...
  return (
    <div className="flex rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] p-6 backdrop-blur-md overflow-hidden relative">
      ...
      {/* 2 Columns Layout */}
      <div className="grid grid-cols-1 md:grid-cols-2 w-full divide-y md:divide-y-0 md:divide-x divide-slate-200 dark:divide-white/10 gap-6 md:gap-0">
        ...
        <div className="flex items-center justify-between md:pr-12 h-full w-full">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">{roadmap.target_role || "Professional Developer"}</h2>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center ml-4">
            <div className="relative flex h-24 w-24 shrink-0 items-center justify-center mb-1">
              <svg className="h-full w-full -rotate-90 transform" viewBox="0 0 100 100">
                <circle cx="50" cy="50" r="40" fill="transparent" stroke="currentColor" className="text-slate-200 dark:text-white/10" strokeWidth="8" />
                <circle
                  cx="50"
                  cy="50"
                  r="40"
        ...
        <div className="flex flex-col justify-center pl-0 md:pl-12 py-4 md:py-0 space-y-3 w-full">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-white">{inProgressTasks} / {totalTasks}</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-white">{pendingTasks} / {totalTasks}</span>
          </div>
        </div>
