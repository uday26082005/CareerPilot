...
export default function LearningRoadmap() {
  const PHASES = [
    ...
    },
    {
      ...
      status: "Pending",
      statusColor: "text-gray-400 border-gray-500/30",
      icon: Database,
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-white/5 bg-[#0a0c1a] p-6 shadow-lg h-full">
      
      {/* Header */}
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-white/5">
        <h2 className="text-sm font-bold text-white">Your Learning Roadmap</h2>
        <div className="flex items-center gap-4">
          <button className="flex items-center gap-1.5 text-[10px] font-medium text-gray-400 hover:text-gray-200 transition-colors">
        ...
        {PHASES.map((phase, idx) => (
          <div key={idx} className="relative flex gap-6 pb-8 last:pb-2">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[9px] font-medium text-gray-500 mb-2">Phase {phase.num}</span>
              <div className={`flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold text-white shadow-lg z-10 ${phase.themeColor}`}>
                {phase.num}
              </div>
              {idx < PHASES.length - 1 && (
                <div className="w-px flex-1 border-l border-dashed border-white/20 mt-2 mb-1" />
              )}
            </div>

            {/* Content Card */}
            <div className="flex-1 rounded-xl border border-white/5 bg-white/[0.01] p-5 hover:bg-white/[0.02] transition-colors mt-6 flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-gray-400 mb-4">{phase.desc}</p>
                
                {/* Skills */}
                <div className="flex flex-wrap gap-2">
                  {phase.skills.map((skill, sIdx) => (
                    <span key={sIdx} className="rounded-md border border-white/5 bg-white/5 px-2 py-1 text-[9px] font-medium text-gray-300">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex flex-col items-end gap-3 shrink-0 ml-4">
                ...
                <span className="text-[10px] text-gray-400">{phase.tasks}</span>
                <button className="mt-4 text-gray-500 hover:text-white transition-colors">
      ...
      <button className="mt-4 w-full rounded-xl border border-dashed border-white/20 bg-white/[0.01] py-4 text-xs font-bold text-gray-400 hover:bg-white/5 hover:text-white transition-colors">
        + Add Custom Milestone
      </button>
