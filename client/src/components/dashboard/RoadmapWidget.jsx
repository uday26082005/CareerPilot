  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-white/5 bg-white/[0.02] p-6 backdrop-blur-md">
      <div className="mb-8 flex items-center justify-between">
        <h3 className="text-lg font-bold text-white">Recommended Roadmap</h3>
        ...
        {steps.map((step, idx) => (
          <div key={idx} className="relative flex flex-col items-center flex-1">
            ...
            {idx !== steps.length - 1 && (
              <div 
                className={`absolute top-5 left-[50%] right-[-50%] h-[2px] -translate-y-1/2 ${
                  step.status === "completed" ? "bg-violet-600" : "bg-white/10 border-t-2 border-dashed border-[#060816]"
                }`}
              />
            )}
            
            {/* Circle */}
            <div className={`relative z-10 flex h-10 w-10 items-center justify-center rounded-full border-2 bg-[#0a0c1a] ${
              step.status === "completed" 
                ? "border-violet-600 bg-violet-600 text-white shadow-[0_0_15px_rgba(139,92,246,0.5)]" 
            ...
            <div className="mt-4 text-center">
              <h4 className="text-xs font-bold text-white max-w-[80px]">{step.title}</h4>
              <p className={`mt-1 text-[10px] uppercase font-bold tracking-wider ${
                step.status === "completed" ? "text-gray-400" :
                step.status === "in_progress" ? "text-violet-400" : "text-gray-500"
              }`}>
