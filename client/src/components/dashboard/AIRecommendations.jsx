    ...
    <div className="flex h-full flex-col rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] p-6 backdrop-blur-md">
      ...
      <div className="flex-1 space-y-3">
        {recommendations.map((rec, idx) => (
          <div key={idx} className="group flex cursor-pointer items-center justify-between rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/[0.02] p-4 transition-colors hover:border-slate-200 dark:border-white/10 hover:bg-white/[0.04]">
            <div className="flex items-center gap-4">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border ${getBg(rec.type)}`}>
                {getIcon(rec.type)}
              </div>
              <div>
                <h4 className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-violet-300 transition-colors">{rec.title}</h4>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-gray-400 line-clamp-1">{rec.desc}</p>
              </div>
            </div>
